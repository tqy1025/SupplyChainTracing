...
# # ================= 辅助函数 =================
#
# def get_domain_vendor(domain):
#     """提取域名主标识，例如 'api.google.com' -> 'google'"""
...
#
#     Rules:
#     1. Ignore minor spelling diffs.
#     2. Recognize aliases (e.g., xiaomi==mi, google==youtube/gstatic, amazon==aws).
#     3. Return 'is_match': true ONLY for same entity.
#
...
#             idx = item.get("id")
#             if idx is not None and idx < len(vendor_pairs):
#                 match_map[vendor_pairs[idx]] = item.get("is_match", False)
#         return match_map
...
#
# def main():
#     # 1. 加载数据 (适配 List of Dict 结构)
#     print(f"[-] Loading data from {INPUT_PICKLE}...")
#     try:
#         raw_data = pd.read_pickle(INPUT_PICKLE)
#
#         # 判断类型并转换为 DataFrame
#         if isinstance(raw_data, list):
#             print(f"    > Detected list format ({len(raw_data)} items). Converting to DataFrame...")
...
#         return
#
#     # 初始化 Domain_A (基于截图中的 'Domain' 列)
#     # 确保 None 被视为 NaN 以便处理
#     df['Domain_A'] = df['Domain']
#
#     # 3. 构建 IP 知识库 (基于已有 Domain 的数据)
#     print("[-] Building IP Knowledge Base...")
#
#     # 筛选出 Domain 不为空的行
#     valid_dns_df = df[df['Domain'].notna() & (df['Domain'] != 'None')].copy()
#
#     tqdm.pandas(desc="Extracting Domain Vendors")
#     valid_dns_df['Domain_Vendor'] = valid_dns_df['Domain'].progress_apply(get_domain_vendor)
#
#     # 知识库结构: IP -> List of Records
#     ip_knowledge = {}
#     # 选取需要的列进行去重 (Device, Remote_IP, Domain)
#     cols_needed = ['Remote_IP', 'Domain', 'Vendor', 'Domain_Vendor']
#     unique_records = valid_dns_df[cols_needed].drop_duplicates()
#
...
#             'domain': row['Domain'],
#             's_vendor': row['Vendor'],  # 设备厂商
#             'd_vendor': row['Domain_Vendor']  # 域名对应的厂商
#         })
#
#     # 4. 准备 LLM 判别 (Rule 2 增强)
#     print("[-] Preparing LLM for Rule 2...")
#
#     # 找出 Domain_A 缺失的行
#     mask_missing = df['Domain_A'].isna() | (df['Domain_A'].astype(str) == 'None')
...
#                 # 只有当字符串不相等且有效时，才交给 LLM
#                 if d_vendor and d_vendor != 'none' and d_vendor != 'nan' and my_vendor != d_vendor:
#                     candidate_pairs.add((my_vendor, d_vendor))
#
#     candidate_list = list(candidate_pairs)
#     print(f"    > Found {len(candidate_list)} unique pairs for LLM.")
#
#     # 批量调用 LLM
#     llm_matches = {}
#     CHUNK_SIZE = 50
#     if candidate_list:
#         for i in tqdm(range(0, len(candidate_list), CHUNK_SIZE), desc="LLM Request"):
#             chunk = candidate_list[i:i + CHUNK_SIZE]
#             res = batch_evaluate_vendor_matches(chunk)
#             llm_matches.update(res)
#
#     # 5. 执行推断 (Rule 1 & 2)
...
#                 is_match = False
#
#                 if d_vendor == my_vendor:  # 精确匹配
#                     is_match = True
#                 elif (my_vendor, d_vendor) in llm_matches:  # LLM 匹配
...
#
#         if found_domain:
#             inferred_domains[idx] = found_domain
#
#     # 填回结果
#     if inferred_domains:
...
#
#     print("-" * 30)
#     print(f"Total Inferred: {len(inferred_domains)}")
#     print(f"Rule 1 (Same Device Vendor): {c1}")
#     print(f"Rule 2 (Device == Domain Vendor): {c2}")
#
#     # 6. 保存
#     print(f"[-] Saving DataFrame to {OUTPUT_FILE}...")
#     df.to_pickle(OUTPUT_FILE)
#
#     # 可选：如果你必须存回 list of dict 格式，取消下面注释
#     # output_list = df.to_dict('records')
#     # pd.to_pickle(output_list, 'Traffic_Cleaned_v3_ListFormat.pkl')
#
#     # 预览
#     if inferred_domains:
#         print("\n[Preview]")
#         sample = list(inferred_domains.keys())[:5]
#         print(df.loc[sample, ['Device', 'Vendor', 'Remote_IP', 'Domain_A']])
#
#     print("\n[-] Done.")
#
#
